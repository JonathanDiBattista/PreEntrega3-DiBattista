...
function mostrarCabanas() {
  for (let i = 0; i < cabañas.length; i++) {
    ...
    let infoCabana = [idCabana, nombreCabana, descripcionCabana, botonAlquilar];
...
function compra() {
  ...
  botonCompra.addEventListener("click", () => {
    ...
    containerCabanas.innerHTML = "";
    let parrafoCompra = `El valor por dia es de 4000, has seleccionado ${diaSeleccionado} dias, asi que tu valor final es: ${
    ...
    botonCompra.addEventListener("click", () => {
      body.innerHTML = "";
      Toastify({
        text: `Gracias ${nombre} por resevar tu estadia`,
    ...
    .then((data) => {
      ...
      for (const comment of data) {
        ...
        commentsContainer.innerHTML += `
          <div class="comments" key="${dataComent.id}">
            <p class="titleComment">${dataComent.name}</p>
            <p class="descriptionComment">${dataComent.description}</p>
          </div>`;
      }
    })
